...
public class GcsSplitCsvFnTest {
  ...
  @Before
  public void setUp() {
    CsvConfiguration.getInstance().withQuote('\"').withDelimiter(',').withRecordSeparator("\n");
  }
  ...
  public static Collection<Object[]> data() {
    return Arrays.asList(new Object[][] {
        ...
        {"test_input_without_quotes.csv", Sets.newHashSet(22L, 551L, 882L)},
        {"test_input_mixed_quotes.csv", Sets.newHashSet(22L, 537L, 1045L, 1595L, 2092L, 2332L)},
  ...
  public void split() {
    ...
    PCollection<KV<String, Set<Long>>> output = input
        .apply(ParDo.of(new TestGcsSplitCsvFn(headersView)).withSideInputs(headersView));
    PAssert.thatSingleton(output).isEqualTo(KV.of(url.getPath(), expectedSplitPoints));
    p.run();
  }

  static class TestGcsSplitCsvFn extends GcsSplitCsvFn {
    TestGcsSplitCsvFn(PCollectionView<Map<String, String[]>> headersView) {
      super(CsvConfiguration.getInstance(), headersView);
      CHUNK_SIZE = 512;
    }
  }
}